...
#Data processor for XGB model
def xgb_processor(regionlist, df, years, splitratio, test_years, target, input_columns, model_path, scalertype = 'MinMax', standard = False, alldata = False):
    #check to make sure the model path exists
    if not os.path.exists(model_path):
        os.makedirs(model_path, exist_ok=True)
    ...
    for region in regionlist:
        ...
        if alldata == False:
            ...
            if standard == True:
                ...
                y_train_reg = y[:split_index]

                x_test_reg = X[split_index:]
                y_test_reg = y[split_index:]

                #add randomization to training data
                xtrain, xtrain2, ytrain, ytrain2 = train_test_split(x_train_reg, y_train_reg, test_size=splitratio, random_state=69)

                #combine randomized data samples
                x_train_reg = pd.concat([xtrain, xtrain])
                y_train_reg = pd.concat([ytrain, ytrain])
        ...
        else: #alldata ==True, we cannot make predictions on the held out data but we are holding out some scenes in HOO, so ok. It would be a good ideas to Hold multiple scenes out - HMO
            
            x_train_reg, x_test_reg, y_train_reg, y_test_reg = train_test_split(X, y, test_size=splitratio, random_state=69)
            
            x_train = pd.concat([x_train, x_train_reg, x_test_reg])
            y_train = pd.concat([y_train, y_train_reg, y_test_reg])
