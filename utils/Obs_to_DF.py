...
def cell_id_2_topography(row, timestamp,transposed_data,nearest_snotel, snotel_data):
    try:
        ...
        selected_snotel_data = snotel_data[cols].copy()
        ...
        transposed_data[cell_id].rename(columns={'dates': 'Date'}, inplace=True)

        
    except Exception as e:
        print(f"{cell_id} throwing error: {e}, moving on...")
...
def process_single_timestamp(args):
    ...
    if timestamp in new_column_names.values():
        print(f"Site processing complete, adding observational data to {timestamp} df...")
        aso_swe_data.apply(lambda row: cell_id_2_topography(row, timestamp, transposed_data, nearest_snotel, snotel_data), axis=1)
...
def Nearest_Snotel_2_obs_MultiProcess(region, output_res, manual, dates):    
    ...
    try:
        ...
        print(f"Go run the get_InSitu_obs script above... ({e})")
    ...
    print(f"Loading {len(aso_swe_files)} processed ASO observations for {region} at {output_res}M resolution")
    ...
    with cf.ProcessPoolExecutor(max_workers=CPUS) as executor:
        futures = {
            executor.submit(process_single_timestamp, (aso_swe_files_folder_path, f, new_column_names, snotel_data, nearest_snotel, Obsdf, obsdf_path, output_res)): f
